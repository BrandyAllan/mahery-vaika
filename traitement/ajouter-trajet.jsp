...
<%
    ...
    if (!"ADMIN".equals(role)) {
        response.sendRedirect("../pages/gestion-trajet.jsp");
        return;
    }
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        if (idDepart != null && idArrivee != null && !idDepart.equals(idArrivee) && tarif != null && !tarif.isEmpty()) {
            ...
            if (gestion.ajouterTrajet(t)) {
                response.sendRedirect("../pages/liste-trajet.jsp?msg=success");
                return;
            } else {
                response.sendRedirect("../pages/ajout-trajet.jsp?error=sql");
                return;
            }
        } else {
            response.sendRedirect("../pages/ajout-trajet.jsp?error=invalid_data");
            return;
        }
    }
    
    response.sendRedirect("../pages/ajout-trajet.jsp");
%>
