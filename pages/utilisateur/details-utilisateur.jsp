    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="mt-3 d-flex flex-wrap gap-2">
                <% if (isAdmin) { %>
                    <a href="modifier-utilisateur.jsp?id=<%= u.getId_utilisateur() %>" class="btn btn-warning">
                        <i class="bi bi-pencil"></i> Modifier
                    </a>
                    <a href="../../traitement/utilisateur/desactiver-utilisateur.jsp?id=<%= u.getId_utilisateur() %>&actif=<%= !u.isActif() %>" class="btn btn-danger" onclick="return confirm('Confirmer ?')">
                        <i class="bi <%= u.isActif() ? "bi-person-x" : "bi-person-check" %>"></i>
                        <%= u.isActif() ? "Desactiver" : "Reactiver" %>
                    </a>
                <% } else { %>
                    ...
                    <button class="btn btn-danger" disabled>
                        <i class="bi bi-person-x"></i> Desactiver
                    </button>
                <% } %>
            </div>
        </div>
